...
- [The little things](issues/10-the-little-things.md) — **stage two, split out of 08.** Ticket 04 folded 05 into 08 because a detail can't be judged apart from the page it lives on; stage one now exists, so that condition is met and the details get their own ticket. Ranked: OG tags and favicon first, then a real focus-visible state, `/llms.txt`, `scroll-margin-top` on the anchors, a print stylesheet, and the marginalia reveal deliberately last. **Five of the six are built**: OG tags and a card generated from the site's own font files by `tools/og-card.py`; a focus ring that replaces the browser default (nothing had ever set `outline`, so keyboard users were getting the lime underline *and* the default ring); `/llms.txt`; scroll margin on the anchors; and a print stylesheet. **What is left is the favicon, which is a question for jass, and the marginalia reveal, which is deliberately last and only if the page still feels dead.** The llms.txt route forced the intro paragraph out of `index.astro` into `src/intro.ts` — three things quote it now, and `TAGLINE` is derived from it rather than restated so the description cannot drift.
...
- **Identity assets** — narrowed to the favicon. The socials half was settled earlier (**they render as text, and all ten SVGs are deleted**), and the OG half is now built: full `og:`/`twitter:` tags in `Layout.astro` and a generated `public/og.png` carrying the wordmark and the address, and deliberately not the title or the description, which both platforms already render as text beside it. What is left is **the favicon, and whether jass's face is it** — `favicon.webp` is his illustrated avatar, `avatar.webp` is committed, a full-frame `photo.webp` exists untracked on main. That is a question for him, not a design problem. See [ticket 10](issues/10-the-little-things.md).
